...
export default function TakePage({ questions, slackUsername }: Props & LoginProps) {
    return <Layout slackUsername={slackUsername}>
        <main>
            ...
            <form action="/api/submit" method="POST">
                ...
                {questions.map((q, i) => <>
                    <Label id={`answer${i + 1}`} sx={{ ":target": { fontWeight: "bold" } }}>
                        {q.question}
                        <br />
                        <Input type="text" name={`answer_${i + 1}`} placeholder={`Type your answer to question ${i + 1} here`} />
                    </Label>
                    <br /><br />
                </>)}
                <Button type="submit" value="Submit" variant="ctaLg">Submit</Button>
                <br /><br />
            </form>
        </main>
    </Layout>
